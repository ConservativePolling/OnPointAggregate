        ...
        const AGGREGATES = [
            { 
                ...
                firstTermPolls: [ 
   { pollster: "Rasmussen Reports", date: "2021-01-22", sampleSize: 1500, approve: 50, disapprove: 48, quality: "A+" },                  
...
/* ——— Georgia (latest → earliest, concise syntax) ——— */
  ...
  { p:"SurveyUSA",                     d:"2024-08-29", s:635,  a:48, b:43, q:"B"  }]
        ...
        // Advanced highlight zoom system
        let highlightZoom = {
        ...
        function optimizedSampling(totalDays, lineDetail) {
            ...
            // Aggressive optimization for zoom ranges
            if (totalDays <= 7) return 1; // Daily for week or less
            if (totalDays <= 30) return Math.max(1, Math.ceil(totalDays / 15)); // 2 weeks max for month
            if (totalDays <= 90) return Math.max(2, Math.ceil(totalDays / 30)); // 3 days max for quarter
            
            const baseInterval = Math.max(MIN_SAMPLING_INTERVAL, Math.min(MAX_SAMPLING_INTERVAL, Math.ceil(totalDays / 300)));
            const detailMultiplier = [3, 2, 1.5, 1, 0.8][lineDetail - 1] || 1;
       ...
       function calculateAllMargins() {
        ...
        function updatePollsterDropdown(){
            ...
            const totalCount = pollsters.reduce((sum, p) => sum + p.count, 0);
            
            const allOption = document.createElement('div');
            allOption.className = 'dropdown-option selected'; 
            allOption.dataset.pollster = 'all';
            allOption.innerHTML = `<i class="fas fa-globe option-icon"></i><span>All Pollsters</span><span class="option-badge">${totalCount}</span>`;
        ...
        function computeAggregationData(primaryPollsForLine) {
            ...
            // Cap maximum timestamps to prevent performance issues
            const maxTimestamps = 500;
            let timestamps = [];
            if (totalDays > 0) {
                const effectiveSamplingRate = Math.max(samplingRate, Math.ceil(totalDays / maxTimestamps));
                for (let i = 0; i < totalDays; i += effectiveSamplingRate) {
                    ...
                    if (timestamps.length >= maxTimestamps) break; // Safety cap
                }
                if (timestamps.length > 0 && effectiveSamplingRate > 1 && timestamps[timestamps.length - 1].getTime() < endDateForAggregation.getTime()) {
        ...
        function startZoomHighlight(event) {
            // Advanced input detection and validation
            const isTouch = event.touches && event.touches.length > 0;
            const isDoubleTouch = isTouch && event.touches.length > 1;
            const button = isTouch ? 0 : event.button;
            
            // Prevent zoom on double touch (pinch) or wrong button
            if (isDoubleTouch || button !== 0) return;
            
            // Multi-layer validation system
            const validationChecks = [
                () => aggregatedData?.timestamps?.length > 0,
                () => chartDimensions?.width > 0 && chartDimensions?.height > 0,
                () => !isProcessing,
                () => !highlightZoom.isHighlighting,
                () => !pinchZoom.active
            ];
            
            if (!validationChecks.every(check => check())) {
                console.warn('Zoom start blocked: validation failed');
                return;
            }
            ...
            if (x < margin.left - 5 || x > width - margin.right + 5 || 
                y < margin.top - 5 || y > height - margin.bottom + 5) {
                return;
            }
            ...
            highlightZoom = {
                ...
                isValid: false,
                cancelled: false,
                touchId: isTouch ? event.touches[0].identifier : null,
                maxWidth: width - margin.left - margin.right
            };
            
            // Store previous zoom state for undo
            ...
            // Initialize visual rectangle
            initializeZoomRectangle(x, y, height, margin);
            
            if(isTouch) event.preventDefault();
            
            console.log('Zoom highlight started', { x, y, isTouch });
        }
        
        function initializeZoomRectangle(x, y, height, margin) {
            const heightRatio = (chartDimensions.yMax - chartDimensions.yMin) / (DEFAULT_Y_MAX - DEFAULT_Y_MIN);
            const h = Math.max(20, (height - margin.top - margin.bottom) * heightRatio);
            ...
            highlightZoomRect.style.width = '0px';
            highlightZoomRect.style.height = `${h}px`;
            highlightZoomRect.style.display = 'block';
            highlightZoomRect.style.opacity = '0.1';
            highlightZoomRect.style.transition = 'opacity 0.1s ease-out';
            
            // Fade in effect
            requestAnimationFrame(() => {
                highlightZoomRect.style.opacity = '0.3';
            });
        }

        function updateZoomHighlight(event) {
            if (!highlightZoom.isHighlighting || highlightZoom.cancelled) return;
            if (!chartDimensions || chartDimensions.width <= 0 || chartDimensions.height <= 0) return;
            
            const isTouch = event.touches && event.touches.length > 0;
            const clientX = isTouch ? event.touches[0].clientX : event.clientX;
            const clientY = isTouch ? event.touches[0].clientY : event.clientY;
            
            // Touch validation for multi-touch scenarios
            if (isTouch) {
                if (event.touches.length > 1) {
                    cancelZoomHighlight('Multi-touch detected');
                    return;
                }
                if (highlightZoom.touchId !== null && event.touches[0].identifier !== highlightZoom.touchId) {
            ...
            // Calculate current position with bounds
            const rawX = clientX - rect.left;
            const rawY = clientY - rect.top;
            const currentX = Math.min(Math.max(margin.left, rawX), width - margin.right);
            
            // Update zoom state
            highlightZoom.currentX = currentX;
            highlightZoom.currentY = rawY;
            
            // Calculate drag distance and time
            const dragDistance = Math.hypot(currentX - highlightZoom.startX, rawY - highlightZoom.startY);
            const dragTime = Date.now() - highlightZoom.startTime;
            
            // Determine if this is a valid drag operation
            const isDragging = dragDistance >= highlightZoom.threshold.distance;
            const hasMinTime = dragTime >= highlightZoom.threshold.time;
            
            if (!highlightZoom.isDragging && isDragging) {
                highlightZoom.isDragging = true;
                console.log('Drag started', { distance: dragDistance, time: dragTime });
            }
            
            // Calculate rectangle dimensions
            const left = Math.min(highlightZoom.startX, currentX);
            const w = Math.abs(currentX - highlightZoom.startX);
            const maxW = Math.min(w, highlightZoom.maxWidth);
            
            // Dynamic visibility and validation
            const isVisible = w >= 3;
            const isValidWidth = w >= highlightZoom.minWidth;
            const isWithinBounds = currentX >= margin.left && currentX <= width - margin.right;
            
            highlightZoom.isValid = isValidWidth && hasMinTime && isWithinBounds;
            
            // Update visual rectangle with enhanced styling
            updateZoomRectangleVisual(left, w, height, margin, yMin, yMax, isVisible, highlightZoom.isValid);
            
            if(isTouch) event.preventDefault();
        }
        
        function updateZoomRectangleVisual(left, width, height, margin, yMin, yMax, isVisible, isValid) {
            ...
            highlightZoomRect.style.width = `${width}px`;
            highlightZoomRect.style.top = `${topOffset}px`;
            highlightZoomRect.style.height = `${h}px`;
            highlightZoomRect.style.display = isVisible ? 'block' : 'none';
            
            // Update CSS classes for professional styling
            highlightZoomRect.classList.remove('valid', 'invalid');
            if (isVisible) {
                if (isValid) {
                    highlightZoomRect.classList.add('valid');
                } else {
                    highlightZoomRect.classList.add('invalid');
                }
            }
        }
        
        function cancelZoomHighlight(reason) {
            console.log('Zoom cancelled:', reason);
            highlightZoom.cancelled = true;
            highlightZoom.isHighlighting = false;
            highlightZoom.isDragging = false;
            highlightZoomRect.style.display = 'none';
            highlightZoomRect.style.transition = '';
            resetZoomState();
        }
        ...
        function endZoomHighlight(event) {
            ...
            // Comprehensive end state validation
            const dragTime = Date.now() - highlightZoom.startTime;
            const wasValid = highlightZoom.isValid;
            const wasDragging = highlightZoom.isDragging;
            const wasCancelled = highlightZoom.cancelled;
            ...
            if (!validateChartState()) {
                console.warn('Zoom failed: Invalid chart state');
                return;
            }
            ...
            const startX = highlightZoom.startX;
            const finalWidth = Math.abs(endX - startX);
            
            // Final validation checks
            const validationResult = validateZoomSelection(startX, endX, finalWidth);
            if (!validationResult.isValid) {
                console.warn('Zoom validation failed:', validationResult.reason);
                return;
            }
            
            // Convert to date range and apply zoom
            const startDate = xToDate(Math.min(startX, endX));
            const endDate = xToDate(Math.max(startX, endX));
            
            if (validateDateRange(startDate, endDate)) {
                executeZoomToRange(startDate, endDate);
                console.log('Zoom applied successfully', { startDate, endDate });
            }
            
            if(isTouch) event.preventDefault();
        }
        
        function cleanupZoomHighlight() {
            highlightZoom.isHighlighting = false;
            highlightZoom.isDragging = false;
            highlightZoomRect.style.display = 'none';
        ...
        function resetZoomState() {
            ...
            highlightZoom.startY = null;
            highlightZoom.currentX = null;
            highlightZoom.currentY = null;
            highlightZoom.startTime = null;
            highlightZoom.isValid = false;
            highlightZoom.cancelled = false;
            highlightZoom.touchId = null;
        }
        ...
        function validateChartState() {
            return chartDimensions && 
                   ...
                   aggregatedData.timestamps.length > 1;
        }
        
        function validateZoomSelection(startX, endX, width) {
            if (width < highlightZoom.minWidth) {
                return { isValid: false, reason: `Width too small: ${width}px < ${highlightZoom.minWidth}px` };
            }
            
            if (Math.abs(startX - endX) < 5) {
                return { isValid: false, reason: 'Selection too narrow' };
            }
        ...
        function validateDateRange(startDate, endDate) {
            if (!startDate || !endDate) {
                console.warn('Invalid dates from xToDate conversion');
                return false;
            }
            
            const timeDiff = Math.abs(endDate - startDate);
            const minTimeRange = 3600000; // 1 hour minimum
            
            if (timeDiff < minTimeRange) {
                console.warn('Time range too small:', timeDiff / 1000 / 60, 'minutes');
                return false;
            }
        ...
        function executeZoomToRange(startDate, endDate) {
            // Use the existing zoom application logic
        ...
        function applyZoomToRange(startDate, endDate) { 
            ...
            // For zoom operations, update immediately without loading screen
        ...
        let mainDropdownInitialized = false;
        function initMainDropdown(){
            ...
            if (!mainDropdownInitialized) {
                ...
                mainDropdownInitialized = true;
            }
        }
        
        let stateDropdownInitialized = false;
...
For questions about methodology, contact: info@onpointaggregate.com`;
        ...
        function initSearch(){
            ...
            // Dynamic intelligent search suggestions
            function getSearchSuggestions(query = '') {
                const currentPolls = getCurrentTermPolls(currentAggregate, currentTerm) || [];
                const uniquePollsters = [...new Set(currentPolls.map(p => p.pollster))].slice(0, 5);
                const recentDates = getRecentDateOptions();
                
                if (!query.trim()) {
                    return [
                        { 
                            title: 'High Quality Polls', 
                            query: 'quality:a+', 
                            shortcut: '⌘Q'
                        },
                        { 
                            title: 'Recent Polls', 
                            query: 'date:last 30 days', 
                            shortcut: '⌘R'
                        }
                    ];
                }

                // Smart contextual suggestions based on what user is typing
                const suggestions = [];
                const q = query.toLowerCase();

                if (q.includes('poll') || q.includes('ras') || q.includes('gal')) {
                    suggestions.push({
                        title: 'pollster:rasmussen',
                        query: 'pollster:rasmussen',
                        shortcut: '↵'
                    });
                }

                if (q.includes('qual') || q.includes('grade') || q.includes('a')) {
                    suggestions.push({
                        title: 'quality:a+',
                        query: 'quality:a+',
                        shortcut: '↵'
                ...
                if (q.includes('date') || q.includes('recent') || q.includes('last')) {
                    suggestions.push({
            ...
            function getRecentDateOptions() {
                ...
                options.push(prevMonth.toLocaleString('default', { month: 'long' }).toLowerCase());
                
                // Common time periods
                options.push('last 7 days', 'last 30 days', 'last 3 months');
                
                return options;
            }

            pollSearch.addEventListener('focus', () => {
                if (pollSearch.value.trim() === '') {
                    showSearchSuggestions();
                }
            });

            pollSearch.addEventListener('input', () => {
                clearTimeout(suggestionTimer);
                const query = pollSearch.value.trim();
                
                if (query === '') {
                    showSearchSuggestions();
                } else if (query.length >= 2) {
                    suggestionTimer = setTimeout(() => {
                        showRelevantSuggestions(query);
                    }, 300);
                } else {
            ...
            pollSearch.addEventListener('blur', (e) => {
            ...
            function showSearchSuggestions() {
                ...
                // Simple, clean entrance
                requestAnimationFrame(() => {
                    searchSuggestions.classList.add('visible');
                });
                
                // Add container spacing
                document.querySelector('.search-container').classList.add('suggestions-active');
            }

            function renderSuggestions(suggestions) {
                // Clear and rebuild the entire suggestions container
                searchSuggestions.innerHTML = '';
                
                // Add header
                const header = document.createElement('div');
                header.className = 'suggestion-header';
                header.textContent = suggestions.length === 0 ? 'No Results' : 'Search Suggestions';
                searchSuggestions.appendChild(header);
                
                // Add suggestions list container
                const listContainer = document.createElement('div');
                listContainer.className = 'suggestions-list';
                listContainer.id = 'suggestionsList';
                searchSuggestions.appendChild(listContainer);
                
                // Update reference
                const suggestionsList = listContainer;
                
                if (suggestions.length === 0) {
                    listContainer.innerHTML = '<div class="suggestions-empty">No matching suggestions found</div>';
                    return;
                }
                ...
                suggestions.forEach((item, index) => {
                    ...
                    suggestionItem.innerHTML = `
                        ...
                        <div class="suggestion-content">
                            ...
                            <div class="suggestion-query">${item.query}</div>
                            ${item.shortcut ? `<div class="suggestion-shortcut">${item.shortcut}</div>` : ''}
                        </div>
                    `;
            ...
            function handleSuggestionSelection(item) {
                ...
                pollSearch.value = item.query;
                clearSearch.style.display = item.query ? 'block' : 'none';
                pollSearch.focus();
                hideSearchSuggestions();
                pollSearch.dispatchEvent(new Event('input'));
            }
            ...
            function positionSearchSuggestions() {
                // Simplified positioning - CSS absolute positioning handles the layout
                // Clear any previous manual positioning styles
                searchSuggestions.style.top = '';
                searchSuggestions.style.left = '';
                searchSuggestions.style.width = '';
                searchSuggestions.style.maxHeight = '';
                
                // Remove old positioning classes
                searchSuggestions.classList.remove('above', 'below');
                
                // Let CSS handle the positioning with absolute positioning
                // The suggestions will always appear below the search input
            }

            function showRelevantSuggestions(query) {
            ...
            pollSearch.addEventListener('keydown', (e) => {
                ...
                switch(e.key) {
                    ...
                    case 'Enter':
                        ...
                        if (selectedSuggestionIndex >= 0 && currentSuggestions[selectedSuggestionIndex]) {
                            handleSuggestionSelection(currentSuggestions[selectedSuggestionIndex]);
                        }
                        break;
            ...
            document.addEventListener('keydown', (e) => {
                if (e.target === pollSearch) return; // Skip if already in search
                ...
                if (e.metaKey || e.ctrlKey) {
                    switch(e.key) {
                        case 'q':
                            e.preventDefault();
                            pollSearch.value = 'quality:a';
                            pollSearch.focus();
                            pollSearch.dispatchEvent(new Event('input'));
                            break;
                        case 'l':
                            e.preventDefault();
                            pollSearch.value = 'sample:>=1500';
                            pollSearch.focus();
                            pollSearch.dispatchEvent(new Event('input'));
                            break;
                        case 'r':
                            e.preventDefault();
                            pollSearch.value = 'date:last 30 days';
                            pollSearch.focus();
                            pollSearch.dispatchEvent(new Event('input'));
                            break;
                        case 'n':
                            if (currentAggregate && currentAggregate.id.includes('approval')) {
                                e.preventDefault();
                                pollSearch.value = 'margin:>0';
                                pollSearch.focus();
                                pollSearch.dispatchEvent(new Event('input'));
                            }
                            break;
                        case 'c':
                            if (currentAggregate && currentAggregate.isRace) {
                                e.preventDefault();
                                pollSearch.value = 'margin:<5';
                                pollSearch.focus();
                                pollSearch.dispatchEvent(new Event('input'));
                            }
                            break;
                    ...
                    if (!['INPUT', 'TEXTAREA'].includes(e.target.tagName)) {
                        switch(e.key) {
                            ...
                            case 'q':
                                e.preventDefault();
                                pollSearch.value = 'quality:';
                                pollSearch.focus();
                                showSearchSuggestions();
                                break;
                            case 'd':
                                e.preventDefault();
                                pollSearch.value = 'date:';
                                pollSearch.focus();
                                showSearchSuggestions();
                                break;
                            case 's':
                                e.preventDefault();
                                pollSearch.value = 'sample:';
                                pollSearch.focus();
                                showSearchSuggestions();
                                break;
                            case 'm':
                                e.preventDefault();
                                pollSearch.value = 'margin:';
                                pollSearch.focus();
                                showSearchSuggestions();
                                break;
                        }
            ...
            function hideSearchSuggestions() {
                ...
                searchSuggestions.classList.add('hiding');
                document.querySelector('.search-container').classList.remove('suggestions-active');
                
                setTimeout(() => {
                    searchSuggestions.style.display = 'none';
                    searchSuggestions.classList.remove('hiding');
                }, 300);
            }

            // Close suggestions when clicking outside
            document.addEventListener('click', (e) => {
                if (!pollSearch.contains(e.target) && !searchSuggestions.contains(e.target)) {
                    hideSearchSuggestions();
                }
            });
        }
        ...
        function xToDate(xPos) {
            if (!aggregatedData.timestamps || aggregatedData.timestamps.length === 0 || !aggregatedData.timestamps[0]) return null;
            if (!chartDimensions || chartDimensions.width <= 0) return null;
            
            const { margin, width } = chartDimensions;
            const chartAreaWidth = width - margin.left - margin.right;
            
            if (chartAreaWidth <= 0) return null;
            
            const firstTimestamp = aggregatedData.timestamps[0].getTime();
            const timeRange = (aggregatedData.timestamps.length > 1) ? (aggregatedData.timestamps.at(-1).getTime() - firstTimestamp) : 86400000; // Default to 1 day
            
            const relativePosition = Math.max(0, Math.min(1, (xPos - margin.left) / chartAreaWidth));
            return new Date(firstTimestamp + relativePosition * timeRange);
        }
        ...
        // Enhanced loading with scroll prevention and timing
        function showLoader() {
        ...
        function hideLoader() {
            // Add a minimum display time for the animations to complete
            setTimeout(() => {
            ...
            }, 4800); // Minimum 4.8 seconds to see full animation sequence
        }

        // Articles Navigation Function
        function navigateToArticles() {
            ...
            `;
            transitionLogo.innerHTML = '<span style="color: var(--text-primary);">ON</span><span style="background: linear-gradient(90deg, #0092ff, #00d4ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">POINT</span><span style="background: linear-gradient(90deg, #e42535, #ff3d71); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 0.6em; margin-left: 10px;">ARTICLES</span>';
            
            transitionOverlay.appendChild(transitionLogo);
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                ...
                // Process data with progress indication
                console.log('Starting data preprocessing...');
                preprocessPollData();
                console.log('Data preprocessing completed');
                
                console.log('Starting aggregate precomputation...');
                precomputeAllAggregates();
                console.log('Aggregate precomputation completed');
                
                console.log('Initializing app...');
                initApp();
                console.log('App initialization completed');
                
                // Set up articles navigation
                const articlesBtn = document.getElementById('articlesNavBtn');
                if (articlesBtn) {
                    articlesBtn.addEventListener('click', navigateToArticles);
                    console.log('Articles navigation set up');
                }
